...
class AckReceiver:
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            print("[Outbound ACK] Connected")
            client.subscribe("persistence/ack")
            print("[Outbound ACK] Subscribed to persistence/ack")
    ...
    def mark_as_processed(self, mongo_id, collection_name=None):
        ...
        if collection_name:
            ...
            print(f"[Outbound ACK] Document {mongo_id} in {collection_name} marked as PROCESSED")
            return
        ...
        for coll in collections:
            ...
            if result.modified_count > 0:
                print(f"[Outbound ACK] Document {mongo_id} in {coll} marked as PROCESSED (found by search)")
                return
